...
Items, Rules, Sitemaps und Persistence Strategies werden in openHAB in einer eigens dafür entwickelten DSL beschrieben. Für alle diese Bereiche werden Konfigurationsdateien im entsprechenden Unterverzeichnis des openHAB Konfigurationsordners abgelegt. Änderungen an den Konfigurationsdateien werden von openHAB zur Laufzeit sofort erkannt und berücksichtigt. Nebst den genannten Bereichen existiert eine globale Konfigurationsdatei, um beispielsweise IP-Adressen für Bindings einzutragen. Folgende Dateien sind für diese Arbeit relevant:

\begin{itemize}
	\item /configurations/openhab.cfg
	\item /configurations/items/demo.items
	\item /configurations/rules/demo.rules
	\item /configurations/sitemaps/demo.sitemap
	\item /configurations/persistence/mqtt.persist
...
\subsubsection{Items}
Die Definition der Items erfolgt in der Datei \lstinline!demo.items! und verwendet die openHAB Item DSL. Details zur Syntax sind auf dem openHAB Wiki (unvollständig) dokumentiert. Siehe \url{https://github.com/openhab/openhab/wiki/Explanation-of-items#syntax}.

Nachfolgend werden die wichtigsten Items anhand von Listing \ref{lst:itemsConfig} erklärt.

\begin{lstlisting}[style=csharp, label={lst:itemsConfig}, caption=demo.items - Konfiguration der Items]
Switch Alarm_active 
	"Alarm-Modus [MAP(alarm.map):%s]"
	<siren>
	(Alarms)
	
Group:Switch:AND(ON, OFF) Alarms
	"Ueberwachung [MAP(alarm.map):%s]"
	<house>

/* Bewegung */
String Motion_Livingroom
	"Bewegungsmelder [MAP(motion.map):%s]"
	{homematic="address=LEQ0797607, channel=1, parameter=MOTION"}
	
String Motion_Livingroom_Sabotage
	"Sabotage"
	{homematic="address=LEQ0797607, channel=1, parameter=ERROR"}

String Motion_Summary
	"Bewegungsmelder [%s]"
	<siren>

/* Lichter */
Switch Hue_1
	"Schlafzimmer [MAP(switch.map):%s]"
	(Lights)
	{hue="1"}
	
Switch Hue_2
	"Wohnzimmer [MAP(switch.map):%s]"
	(Lights)
	{hue="2"}
...
/* Fenster */
String Window_Bedroom
	"Schlafzimmerfenster [MAP(window.map):%s]"
	<contact>
	(Windows)
	{homematic="address=LEQ1469091,channel=1,parameter=STATE"}
	
Group:String:OR(true, false) Windows
	"Fenster [%d offen]"
	<contact>
\end{lstlisting}


\textbf{Kontaktsensor, Zeile 38 - 42}\\
...
\textbf{Bewegungsmelder, Zeile 11 - 21}\\
...
Leider muss bei Homematic jeder Parameter einem eigenen Item zugeordnet werden. Deshalb haben wir ein drittes String-Item \lstinline!Motion_Summary! definiert, in dem die Werte der beiden anderen Items zusammengefasst werden. 

\textbf{Lichter, Zeile 24 - 32}\\
Die Philips Hue Lampen können direkt an ein Switch Item gekoppelt werden. Im Bindingparameter muss lediglich die Nummer der Birne eingetragen werden. Der Status (\lstinline!ON/OFF!) wird mit der Transformation-Map in «Ein» bzw. «Aus» übersetzt.

\textbf{Alarm, Zeile 1 - 4}\\
...
sitemap demo label="Ihr Haus" {
	...
	Group item=Lights icon="light" {
		...
		Frame label="Suedseite" {
			Switch item=Hue_2
		}
	}
	Group item=Alarms {
		Frame label="Alarm" {
			Switch item=Alarm_active
		}
		Frame label="Bewegung" {
			Text item=Motion_Summary
		}
		Frame label="Kamera" {
			Webview url="http://192.168.1.15:80/jpg/1/image.jpg"
		}
	}
}
...
\subsubsection{Rules}
Wie im Lösungskonzept beschrieben, werden Rules zur Automatisierung von Aktionen eingesetzt. Die Regeln können im File \lstinline!/configurations/rules/demo.rules! definiert werden. \\\\
\textbf{Alarm} \\
...
Das Protokoll wurde speziell für Systeme designt, die über wenig Speicherplatz und kleine Netzwerk-Bandbreite verfügen, was bei IoT-Anwendungen meist der Fall ist.
...
Das Deploymentdiagramm (Abbildung \ref{fig:deploymentDiagram}) zeigt, wie der Cloudservice und das Smart-Home aufgebaut sind. Der Cloudservice umfasst eine virtuelle Maschine, eine Worker Role und den Storage. Auf der virtuellen Maschine läuft der Broker. Eine Worker Role hört auf ein Topic, erhält die Nachrichten vom Broker und speicher sie in den Cloudstorage.\\
Auf dem Raspberry Pi werden über das MQTT-Binding des openHABs die Item-Updates an den Broker, in die Cloud, gesendet.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.6]{report/img/deployment}
...
Da die MQTT-Verbindung verschlüsselt werden soll, müssen verschiedene Zertifikate erstellt werden. Das erstellte Server-Zertifikat muss von einer CA (Certification Authority) signiert werden. Da ein gültiges Zertifikat nicht entgeltlich erworben werden möchte, wird eine eigene Zertifizierungsstelle erstellt. OpenSSl bringt dafür alle nötigen Mittel für die Erzeugung eines CAs mit. Dies bringt den Nachteil mit, dass Computersysteme diesem Zertifikat nicht automatisch trauen. Daher muss das Zertifkat von Hand dem Certificate Store als «Trusted Root Certification Authority» hinzugefügt werden. \\ \\
...
Wie bereits im Lösungskonzept erarbeitet, wird als MQTT-Broker «Mosquitto» eingesetzt. Der Broker kann als Binary installiert und über die Commandline gestartet werden.
...
\begin{tabularx}{\textwidth}{XX}
		...
		\\ \hline
			...
			IP-Adresse, an die der Default-Listener gebunden wird.
		\\ \hline
			port 8883 &
...
Um den Broker zu starten, muss in der Konsole (cmd.exe) in das Installationsverzeichnis (\path{C:\ProgramFiles(x86)\mosquitto\)} navigiert werden. Dort kann der Broker mit der angepassten Konfiguration gestartet werden. Damit in der Konsole Feedback angezeigt wird, wird der Broker im «Verbose-Modus» gestartet: \\
...
Im Lösungskonzept haben wir bereits erklärt, dass wir die Events auf dem openHAB Event Bus via MQTT an die Cloud übertragen wollen, und dass es ein MQTT Plugin für openHAB gibt. Nachdem der Broker installiert worden ist, wollen wir nun unsere openHAB Items an den Broker senden. Dazu installieren wir das openHAB MQTT Persistence Plugin auf unserem Raspberry Pi: \\
...
\begin{lstlisting}[style=csharp, caption=Auszug /configuration/openhab.cfg - MQTT Config]
...
\begin{lstlisting}[style=csharp, caption=/configuration/mqtt.persist]
Strategies {
	everyMinute : "1 * * * * ?"
	everyHour : "0 0 * * * ?"
	default = everyChange
...
In der \lstinline!OnStart()!-Methode der Worke Role werden zuerst die Referenzen zum Table- und Blob-Storage erzeugt. \\
...
Im Sequenzdiagramm der Abbildung \ref{fig:sequenzMQTT} wird aufgezeigt, wie die MQTT-Nachrichten nach dem QoS 2 versendet werden.
...
\\ \\
Sobald Eclipse vollständig eingericht war, kann ein Action Skeleton anhand eines Maven Archetypes generiert werden. Eine Anleitung dazu befindet sich im openHAB GitHub Wiki: \url{https://github.com/openhab/openhab/wiki/How-To-Implement-An-Action}. \\
...
Nachdem wir die MQTT Action programmiert hatten, mussten wir sie mit Hilfe von Eclipse als .jar Datei exportieren. Danach haben wir in der Konfigurationsdatei von openHAB (openhab.cfg) den Eintrag für die MQTT Action gemacht. Diese verweist auf den selben Broker, den wir auch für das Persistence Plugin benötigen.
...
\label{sec:androidRealization}
Im Lösungskonzept im Abschnitt \ref{sec:androidArchSolution}, ab Seite \pageref{sec:androidArchSolution}, haben wir bereits erklärt, wie die Architektur der App aussieht. In diesem Abschnitt gehen wir auf die eingesetzten Frameworks, wichtige Klassen und das User Interface Design ein.
...
\textbf{Butterknife} \\
Butterknife ist eine nützliche kleine Library zur Dependency Injection von View Komponenten in Activities. Jede Activity besitzt in der Regel eine Root-View, die in einem Layout Resource File mit XML definiert wird. In diesen Layout Files befinden sich typischerweise weitere User Interface Elemente, wie Buttons, Input-Felder oder Bilder. Die Activity ist dazu gedacht, jene Elemente mit Code zu beleben, also Events zu behandeln, Texte zu aktualisieren und so weiter. Die Referenzen auf diese Elemente muss sich die Activity selbst beschaffen, indem sie die \lstinline!findViewById(int id)! Methode aufruft und das Element anschliessend einer lokalen Variable oder Instanzvariable zuweist. Die Methode \lstinline!findViewById(int id)! hat den Rückgabetyp \lstinline!View!, deshalb müssen alle Element explizit gecastet werden.
...
Das App wurde nach den Prinzipien des Material Designs gestaltet. Google bietet dazu Guidelines an: \url{https://www.google.com/design/spec/material-design/introduction.html}. Insbesondere haben wir versucht, die strengen Rastervorgaben mit Hilfe von Margins und Paddings einzuhalten, damit für das Auge ein harmonischer Eindruck entsteht. Alle verwendeten Farben stammen aus den offiziellen Material Design Farbpaletten.

\begin{itemize}
	\item Primary Color: Indigo 500 (\lstinline!#3F51B5!)
	\item Primary Color Dark: Indigo 700 (\lstinline!#303F9F!)
	\item Accent Color: Cyan 500 (\lstinline!#00BCD4!)
\end{itemize}
...
Da die Komponenten nicht in die Infrastruktur der Schule integriert werden durften, musste auf ein eigenes, lokales Netzwerk zurückgegriffen werden. Dabei handelt es sich um ein unabhängiges Netzwerk, auf das von externer Seite nicht zugegriffen werden kann. \\
Dies stellt einige Herausforderungen an die Erreichbarkeit unseres Systems. Einerseits die Konnektivität mit der Cloud und andererseits das Erreichen von openHAB durch unseren Android Client. \\
Durch das Anbinden des Raspberry Pi an das lokale und das HSR-Netzwerk wird die Erreichbarkeit der Cloud gewährleistet. Es sind also zwei Network-Interfaces konfiguriert. Eines für das lokale und eines für das HSR Netzwerk. Damit die MQTT-Nachrichten an das richtige Interface gesendet wird, muss die Default Route auf das HSR-Interface (eth0) gesetzt werden: \lstinline!ip route add default dev eth0!. \\\\
